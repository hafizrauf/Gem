...
class Doduo:
    def __init__(self, args=None, basedir="./"):
        self.device = torch.device("cpu")  # Using CPU for simplicity

        if args is None:
            args = argparse.Namespace()

        self.args = args
        self.args.shortcut_name = "bert-base-uncased"  # Use base BERT
        self.args.batch_size = 16

        # Load tokenizer and BERT model
        self.tokenizer = BertTokenizer.from_pretrained(self.args.shortcut_name)
        self.coltype_model = BertModel.from_pretrained(self.args.shortcut_name).to(self.device)
    ...
    def annotate_columns(self, df: pd.DataFrame):
        ...
        # Store embeddings in AnnotatedDataFrame
        adf.colemb = embeddings_list
        return adf
...
if __name__ == "__main__":
    ...
    # Extract embeddings
    embeddings = np.array(annotated_df.colemb)
