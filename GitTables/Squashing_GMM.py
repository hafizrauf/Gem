...
def prototype_induction(all_values, num_prototypes=50):
...
def similarity_function(n, prototypes):
    ...
    # Avoid division by zero: set a small positive value where differences are zero
    differences = np.where(differences == 0, 1e-10, differences)
    return np.power(differences, -1.0)
...
def calculate_embeddings(continuous_cols, prototypes):
    ...
    for i, col in tqdm(enumerate(continuous_cols), total=len(continuous_cols), desc='Calculating embeddings'):
        col_embeddings = np.array([similarity_function(n[0], prototypes) for n in col if n[0] is not np.nan and n[0] != 0])
        embeddings[i, :] = np.mean(col_embeddings, axis=0) if col_embeddings.size > 0 else np.zeros(dim)
    return embeddings
